...
catastrophic failure events, such as large, rapid drawdowns are rare under normal market conditions, making naïve Monte Carlo (MC) sampling computationally expensive. We apply Adaptive Stress Testing (AST), a reinforcement learning
...
budget of A1, to give the adversary ample opportunity to discover exploits
...
Two important considerations for IS validity. First, the likelihood penalty
$-0.05\,\varepsilon_t^2$ in the reward (Eq.~3) penalizes
$\mathbb{E}[\varepsilon_t^2] = \mu_t^2 + \sigma_t^2$, which
encourages the policy to learn a \emph{mean} closer to zero, not merely to
reduce the magnitude of individual samples. These are distinct because only the
former changes the IS denominator $\log\pi_\theta(\varepsilon_t\mid s_t)$ via
the learned mean parameter. Crucially, because stochastic action sampling is
used at evaluation time (not the deterministic mean), $\log\pi_\theta$ is
evaluated under the exact same distribution from which actions are drawn,
preserving IS validity regardless of how the penalty shapes the learned mean
versus variance (see Sec.~\ref{sec:limitations} for further discussion).
Second, \texttt{evaluate\_actions} in SB3 returns log-probabilities under the
unconstrained Gaussian $\mathcal{N}(\mu_t,\sigma_t^2)$, not the truncated
distribution induced by clipping actions to the $[-3,3]$ interval. For V2, where the
adversary's mean actions are moderate, this approximation is negligible. For
V1, where actions frequently approach the $\pm 3$ boundary, the pre-clipping
log-prob can differ from the true truncated log-prob, introducing a systematic
(though likely small) bias in IS weights.
...
SMA crossover signal can trigger an exit. The zero standard deviation in V1+AST time-to-failure reflects the deterministic structure of the learned attack: A1 consistently triggers the 5-step exit lag condition within the same window regardless of the stochastic action draws.
...
($\pm 7.7 \times 10^{-12}$; see Limitations for further discussion): these failures require extremely non-nominal
...
(exploitation of exit lag via quick-acting crashes), which directly
...
A further limitation concerns IS weight degeneracy for V1. The V1
adversary operates at a mean NLL of $4.868$ nats/step versus the
nominal $1.420$, producing per-trajectory importance weights on the
order of $e^{-345}$ over a 100-step episode. Here, the
effective sample size:
\[
  \mathrm{ESS} = \frac{\bigl(\sum_i w^{(i)}\bigr)^2}{\sum_i \bigl(w^{(i)}\bigr)^2}
\]
is likely in the single digits, meaning the IS estimate of
$1.5\!\times\!10^{-11}$ rests on very few effective trajectories and
should be interpreted as an order-of-magnitude indicator rather than a
precise probability. The V2 estimate ($2.0\!\times\!10^{-3}$, NLL gap
of only $0.14$ nats/step) does not suffer from this issue to the same
degree.

A second IS validity caveat concerns action-space clipping.
The adversary's action space is $\varepsilon_t\in[-3,3]$, but the PPO policy
is an unconstrained Gaussian; SB3 clips sampled actions to $[-3,3]$ before
stepping the environment. The log-probabilities returned by
\texttt{evaluate\_actions} are computed under the \emph{pre-clipping}
Gaussian, not under the truncated distribution that describes the actual
sampling procedure. For V1, the adversary saturates near the $\pm 3$
boundary (avg.\ NLL $4.868$ nats/step $\approx 3\sigma$ actions), so a
non-trivial probability mass lies outside $[-3,3]$ and the pre-clipping
log-prob understates $\log\pi_\theta(\varepsilon_t\mid s_t)$, biasing IS
weights upward. For V2, whose mean actions are far from the boundary (NLL
$1.561$ nats/step), the truncation effect is negligible. Correcting this
bias would require replacing the Gaussian with an explicitly truncated
normal or using a tanh-squashed policy, and is left for future work.
